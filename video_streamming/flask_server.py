...
# USAGE
# python flask_server.py --ip 0.0.0.0 --port 8000
...
def videoWriter_setup(vidcap: 'cv2.VideoCapture') -> 'cv2.VideoWriter':
    '''
    create test vizulization folder if not exist
    set videoWriter by videoCapture settings
    '''
    test_video_path = f'initial_{now()}_raw.avi'
    fps = int(vidcap.get(cv2.CAP_PROP_FPS))
    # fps = 25
    size = (int(vidcap.get(cv2.CAP_PROP_FRAME_WIDTH)),
            int(vidcap.get(cv2.CAP_PROP_FRAME_HEIGHT)))
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    videoWriter = cv2.VideoWriter(test_video_path, fourcc, fps, size)
    return videoWriter
...
writer = videoWriter_setup(vidcap=vs.stream.stream)
